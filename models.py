...
pos_weight = float(h * l - association_nam) / (association_nam)
...
class GAutoencoder(Model,):
    ...
    '''def _loss(self):
        for var in self.layers[0].vars.values():
            self.loss += FLAGS.weight_decay * tf.nn.weighted_cross_entropy_with_logits(logits=self.outputs, targets=self.placeholders['labels'] , pos_weight=pos_weight)  # 防止过拟合，权重衰减*利用l2范数来计算张量误差值
        for var in self.layers[1].vars.values():
        self.loss += FLAGS.weight_decay * tf.nn.weighted_cross_entropy_with_logits(logits=self.outputs, targets=self.placeholders['labels'] , pos_weight=pos_weight)
    ...
    def _loss(self):
        for var in self.layers[0].vars.values():
            self.loss += FLAGS.weight_decay * tf.nn.l2_loss(var)#防止过拟合，权重衰减*利用l2范数来计算张量误差值
        for var in self.layers[1].vars.values():
